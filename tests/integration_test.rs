...
fn test_detector() -> Result<(), anyhow::Error> {
    ...
    assert!(detector.get_all_detected_games().is_some());
    assert!(detector.get_all_detected_games_with_box_art().is_some());
    assert!(detector.get_all_detected_games_per_launcher().is_some());
